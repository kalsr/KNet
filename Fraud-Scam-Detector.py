...
# Fraud/Scam-Detector (STREAMLIT SAFE VERSION)
...
Fraud / Scam Detection Platform<br>
Designed & Developed by <b>Randy Singh</b> – <b>KNet Consulting Group</b>
</div>
""", unsafe_allow_html=True)

st.markdown("### 🚨 Enterprise Fraud & Scam Detection System")
...
with tab2:
    ...
    if img_file:
        ...
        if OCR_AVAILABLE:
            ...
            st.error("OCR engine not available in this environment.")
            st.info("Deploy locally or use a cloud OCR API.")

    if st.button("Generate Demo Screenshot"):
        st.image(sample_image(), caption="Generated Scam Screenshot")
...
with tab3:
    ...
    for _ in range(n):
        t = sample_text()
        r, s = detect_fraud(t)
        demo_data.append([t, s, r])

    df_demo = pd.DataFrame(demo_data, columns=["Text", "Score", "Result"])
