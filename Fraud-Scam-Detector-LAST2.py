...
def generate_sample_image():
    ...
    if font:
        d.text((50,350), text, fill="black", font=font)
    else:
        d.text((50,350), text, fill="black")
    return img, text
...
def llm_explain(text):
    if any(k in text.lower() for k in KEYWORDS):
        return "LLM Analysis: This message shows multiple scam indicators."
    else:
...
with tab2:
    ...
    with col2:
        ...
        if st.button("Generate & Analyze Sample Screenshots"):
            for _ in range(n):
                img, text = generate_sample_image()
                st.image(img, width=1200)
                res, score, conf, risk = rule_score(text)
                ml = ml_predict(text)
                llm = llm_explain(text)
...
with tab5:
    if not st.session_state.results.empty:
        ...
        for _, r in st.session_state.results.iterrows():
            pdf.multi_cell(
                0,10,
                f"{r['Timestamp']} | {r['Source']} | {r['Result']} | Risk: {r['Risk']} | ML: {r['ML']}\n{r['Content']}\nLLM: {r['LLM_Explanation']}\n"
            )
