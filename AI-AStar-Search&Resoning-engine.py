...
CASE_INFO = {
    ...
    "ASTAR": {
        ...
        "use_cases": [
            ...
            "Prioritized incident response path finding when search speed matters",
            "Optimal path search guided by domain knowledge encoded as a heuristic"
        ],
    },
}
...
def compute_heuristic_table(graph, goal, weight_field="Cost"):
    ...
    edge_weights = [data.get(weight_field, 1.0) for _, _, data in graph.edges(data=True)]
    min_edge_weight = min(edge_weights) if edge_weights else 1.0
    min_edge_weight = max(min_edge_weight, 0.0)

    heuristic = {}
    for node in graph.nodes():
        hops = hop_distances.get(node)
        heuristic[node] = (hops * min_edge_weight) if hops is not None else 0.0
    return heuristic
...
def astar_search(graph, start, goal, weight_field="Cost"):
    ...
    while frontier:
        f_score, _, node, path, g_score = heapq.heappop(frontier)
        if node in visited:
            continue
        visited.add(node)
        visited_order.append(node)
        ...
        for neighbor in sorted(graph.successors(node)):
            if neighbor not in visited:
                ...
                new_f = new_g + heuristic.get(neighbor, 0.0)
                counter += 1
...
def build_search_reasoning_prompt(case_key, graph_text, start, goal, weight_field="Cost", heuristic=None):
    base = (
    ...
    if case_key == "DFS":
        ...
        heuristic_lines = "\n".join(
            str(node) + ": " + str(round(value, 2)) for node, value in heuristic.items()
        )
        base += (
            "Precomputed admissible heuristic h(n), the estimated remaining '" +
            weight_field + "' cost from node n to the goal. Use these exact "
            "values, do not recompute them:\n" + heuristic_lines + "\n\n"
            "Algorithm: A* Search using a min priority queue ordered by "
            "f(n) = g(n) + h(n), where g(n) is the cumulative '" + weight_field +
            "' cost from the start to n along the path found so far, and h(n) "
            "is the precomputed heuristic value given above. Always expand the "
            "unvisited node with the lowest f(n), breaking ties by lowest g(n) "
            "and then by alphabetical node ID. Stop as soon as the goal node is "
            "popped as the current minimum.\n"
            "Return strictly this JSON shape and nothing else:\n"
            "{\"found\": true or false, \"path\": [\"start\", \"...\", \"goal\"], "
            "\"visited_order\": [\"...\"], \"cost\": g_of_goal_number}"
        )
    else:
        base += (
...
def run_llm_reasoning(case_key, graph, nodes_df, edges_df, start, goal, weight_field, provider, api_key, model, heuristic=None):
...
def render_case(case_key):
    ...
    weight_field = "Cost"
    if case_key in ("UCS", "ASTAR"):
        weight_field = st.selectbox(
            "Cost field used by " + info["full_name"],
...
def main():
    ...
    tab_dfs, tab_bfs, tab_ucs, tab_astar = st.tabs([
        ...
        "A* -- Heuristic-Guided Optimal Path",
    ])
