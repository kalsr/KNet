...
LIGHT_BG = "#F4F7FB"
DEV_BLUE = "#4FA8FF"
TITLE_BLUE = "#38BDF8"  # Bold blue used for the top header title
...
def validate_schema(df: pd.DataFrame, schema: dict, module_label: str) -> bool:
    ...
    module's analysis needs before we try to score it. Prevents KeyError
    crashes when an uploaded CSV doesn't match the expected schema (or when a
    stale dataframe from a different mode/module ends up in session state).
    """
    required = list(schema.keys())
    missing = [c for c in required if c not in df.columns]
    if missing:
        st.error(
            f"**{module_label}: the loaded data is missing required column(s): "
            f"{', '.join(missing)}.**\n\n"
            "Please download the CSV template above and match its columns exactly, "
            "or switch to 'Generate Synthetic Data' and click **Generate Synthetic Data** again."
        )
...
def data_source_controls(key_prefix, schema, gen_func, default_n=200, template_cols=None,
    ...
    NOTE: synthetic-generated data and uploaded data are stored under separate
    session_state keys (`<prefix>_gen_df` / `<prefix>_upload_df`). Previously
    both modes shared a single `<prefix>_df` key, so switching the radio button
    back to "Generate Synthetic Data" without clicking Generate again would
    silently reuse a stale *uploaded* dataframe (which could be missing columns
    the analysis needs, e.g. "severity") and crash downstream with a KeyError.
    """
    ...
    if mode.startswith("Generate"):
        ...
        if st.button("Generate Synthetic Data", key=f"{key_prefix}_gen_btn", type="primary"):
            df = gen_func(n)
            st.session_state[f"{key_prefix}_gen_df"] = df
        if f"{key_prefix}_gen_df" in st.session_state:
            df = st.session_state[f"{key_prefix}_gen_df"]
    else:
...
def render_soc():
    ...
    if df is None or df.empty:
        ...
        return
    if not validate_schema(df, SOC_SCHEMA, "Autonomous SOC"):
        return
...
def render_vuln():
    ...
    if df is None or df.empty:
        st.info("Generate synthetic data or upload a CSV to run the AI analysis.")
        return
    if not validate_schema(df, VULN_SCHEMA, "AI Vulnerability & Exposure Management"):
        return
...
def render_behav():
    ...
    if df is None or df.empty:
        st.info("Generate synthetic data or upload a CSV to run the AI analysis.")
        return
    if not validate_schema(df, BEHAV_SCHEMA, "Behavioral Threat & Anomaly Detection"):
        return
...
def render_ir():
    ...
    if df is None or df.empty:
        st.info("Generate synthetic data or upload a CSV to run the AI analysis.")
        return
    if not validate_schema(df, IR_SCHEMA, "AI Incident Response & Attack-Path Analysis"):
        return
...
def render_identity():
    ...
    if df is None or df.empty:
        st.info("Generate synthetic data or upload a CSV to run the AI analysis.")
        return
    if not validate_schema(df, ID_SCHEMA, "AI Identity & Agent Security"):
        return
