...
<title>KNet 5G Intelligence Framework</title>
<script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;900&display=swap');
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#060E1E;color:#CFE2FF;font-family:'Inter',sans-serif;min-height:100vh}

  /* ── HERO ── */
  .hero{background:linear-gradient(135deg,#0A1628 0%,#0D2B6B 55%,#1565C0 100%);
    padding:42px 48px 32px;box-shadow:0 10px 50px rgba(21,101,192,.5)}
  .hero-title{font-size:2.4rem;font-weight:900;color:#1E90FF;letter-spacing:-.5px;line-height:1.15}
  .hero-sub{font-size:1rem;color:#90CAF9;margin-top:8px}
  .hero-badge{display:inline-block;background:rgba(30,144,255,.18);border:1px solid #1E90FF;
    color:#64B5F6;padding:5px 16px;border-radius:20px;font-size:.82rem;font-weight:700;
    margin-top:14px;letter-spacing:.5px}

  /* ── NAV TABS ── */
  .tabs{display:flex;gap:4px;padding:16px 24px 0;background:#0A1628;
    border-bottom:2px solid #1565C0;flex-wrap:wrap}
  .tab{padding:10px 18px;border-radius:8px 8px 0 0;cursor:pointer;font-weight:600;
    font-size:.85rem;color:#90CAF9;border:1px solid transparent;
    border-bottom:none;transition:all .2s}
  .tab:hover{background:#1565C020;color:#64B5F6}
  .tab.active{background:#0D2B6B;color:#1E90FF;border-color:#1E90FF;border-bottom:2px solid #0D2B6B}

  /* ── PANELS ── */
  .panel{display:none;padding:30px 40px;animation:fadeIn .3s ease}
  ...
  .kpi-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:14px;
    margin-bottom:28px}
  .kpi{background:linear-gradient(135deg,#0D2B6B,#1565C0);border-radius:12px;
    padding:18px 16px;text-align:center;border:1px solid rgba(30,144,255,.3)}
  .kpi-val{font-size:1.7rem;font-weight:900;color:#1E90FF}
  .kpi-lbl{font-size:.75rem;color:#90CAF9;margin-top:4px}

  /* ── TAXONOMY BOX ── */
  .tax{background:#0A1628;border:1px solid #1565C0;border-radius:10px;
    padding:20px 24px;line-height:1.75;color:#CFE2FF;font-size:.9rem;margin-bottom:20px}
  .tax b{color:#64B5F6}
  .tax ul{margin-left:18px;margin-top:6px}

  /* ── TABLE ── */
  .tbl-wrap{overflow-x:auto;border-radius:10px;border:1px solid #1565C0;margin-bottom:24px}
  table{border-collapse:collapse;width:100%;font-size:.78rem}
  thead tr{background:#0D2B6B}
  thead th{padding:10px 12px;color:#1E90FF;font-weight:700;text-align:left;
    white-space:nowrap;border-bottom:2px solid #1E90FF}
  tbody tr:nth-child(odd){background:#0A1628}
  tbody tr:nth-child(even){background:#0D2148}
  tbody td{padding:8px 12px;color:#B0C4DE;white-space:nowrap}

  /* ── FIELD CARDS ── */
  .field{background:#0D2148;border-left:3px solid #1E90FF;border-radius:6px;
    padding:13px 16px;margin-bottom:10px;font-size:.875rem}
  ...
  /* ── MERMAID ── */
  .mermaid-wrap{background:#0D2148;border-radius:10px;padding:20px;
    border:1px solid #1565C0;margin-bottom:24px;overflow-x:auto}
  .mermaid-wrap h4{color:#64B5F6;margin-bottom:12px;font-size:.95rem}

  /* ── CONTROLS ── */
  .ctrl-row{display:flex;gap:14px;align-items:center;margin-bottom:24px;flex-wrap:wrap}
  .ctrl-row label{color:#90CAF9;font-size:.85rem;font-weight:600}
  input[type=range]{accent-color:#1E90FF;width:200px}
  .btn{background:#1565C0;color:#fff;border:none;padding:9px 22px;
    border-radius:8px;cursor:pointer;font-weight:700;font-size:.85rem;
    transition:background .2s}
  .btn:hover{background:#1E90FF}
  .btn.danger{background:#7B1A1A}
  ...
  .stat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;
    margin-bottom:24px}
  .stat-card{background:#0D2148;border:1px solid #1565C020;border-radius:8px;
    padding:12px 16px}
  .stat-key{font-size:.75rem;color:#90CAF9;margin-bottom:4px}
  .stat-val{font-size:1.1rem;font-weight:700;color:#64B5F6}

  /* ── EXPORT ── */
  .export-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px}
  .export-card{background:#0D2B6B;border:1px solid #1E90FF30;border-radius:10px;
    padding:18px;text-align:center;cursor:pointer;transition:all .2s}
  .export-card:hover{border-color:#1E90FF;background:#1565C020}
  .export-icon{font-size:2rem;margin-bottom:8px}
  .export-lbl{color:#64B5F6;font-weight:700;font-size:.9rem}
  .export-sub{color:#607D8B;font-size:.77rem;margin-top:4px}

  /* ── FOOTER ── */
  .footer{text-align:center;padding:20px;color:#37474F;font-size:.8rem;
    border-top:1px solid #1565C020;margin-top:40px}
</style>
</head>
<body>
...
<!-- HERO -->
<div class="hero">
  <div class="hero-title">📡 KNet 5G Intelligence Framework</div>
  <div class="hero-sub">Advanced 5G Network Analytics · Taxonomy · Diagnostics · Export Platform</div>
  <div class="hero-badge">🏢 Developed by Randy Singh &nbsp;|&nbsp; Kalsnet (KNet) Consulting Group</div>
</div>

<!-- TABS -->
<div class="tabs">
  <div class="tab active" onclick="showTab('overview')">🏠 Overview</div>
  <div class="tab" onclick="showTab('data')">📊 Data Explorer</div>
  <div class="tab" onclick="showTab('diagrams')">🗺️ Diagrams</div>
  <div class="tab" onclick="showTab('fields')">📖 Field Dictionary</div>
...
<div id="panel-overview" class="panel active">
  ...
  <div class="sh">📡 5G Taxonomy</div>
  <div class="tax">
    <b>5G (Fifth Generation)</b> is the global wireless standard defined by <b>ITU IMT-2020</b>
    and implemented via <b>3GPP Release 15–18</b>. It delivers three foundational service pillars:
    <br><br>
    <b>① eMBB — Enhanced Mobile Broadband</b><br>
    Peak downlink ≥ 20 Gbps. Powers AR/VR, 4K/8K streaming, holographics.
    <br><br>
    <b>② URLLC — Ultra-Reliable Low-Latency Communications</b><br>
    Latency ≤ 1 ms, reliability ≥ 99.9999%. Powers autonomous vehicles, remote surgery.
    <br><br>
    <b>③ mMTC — Massive Machine-Type Communications</b><br>
    ≥ 1 million devices/km². Powers smart cities, agriculture IoT, industrial sensors.
    <br><br>
    <b>Key Architecture Pillars:</b>
    <ul>
      <li><b>Service-Based Architecture (SBA)</b> — microservice 5G Core with AMF, SMF, UPF, PCF</li>
      <li><b>Network Slicing</b> — isolated logical networks on shared physical infrastructure</li>
      <li><b>Open RAN (O-RAN)</b> — disaggregated vendor-neutral radio networks</li>
      <li><b>Multi-Access Edge Computing (MEC)</b> — compute at the edge for ultra-low latency</li>
      <li><b>Spectrum Strategy</b> — Sub-6 GHz coverage + mmWave capacity + DSS LTE coexistence</li>
      <li><b>Massive MIMO / Beamforming</b> — 64T64R+ antenna arrays for spatial multiplexing</li>
    </ul>
  </div>

  <div class="sh">🏢 KNet Framework Philosophy</div>
  <div class="tax">
    The <b>KNet 5G Intelligence Framework</b> by <b>Randy Singh</b> at
    <b>Kalsnet (KNet) Consulting Group</b> is built on five pillars:
    <ol style="margin-left:18px;margin-top:8px">
      <li><b>Performance Benchmarking</b> — KPI baselines vs ITU IMT-2020 targets</li>
      <li><b>Use-Case Alignment</b> — matching network slice to application SLA</li>
      <li><b>Spectral Intelligence</b> — FR1 vs FR2 band strategy per geography</li>
      <li><b>Energy Optimisation</b> — Green 5G: Mbps/Watt as a first-class KPI</li>
      <li><b>Vendor-Agnostic Analytics</b> — O-RAN aligned, multi-vendor QoS scoring</li>
    </ol>
  </div>

  <div class="sh">📊 Live KPI Summary</div>
...
<div id="panel-data" class="panel">
  <div class="sh">⚙️ Data Controls</div>
  <div class="ctrl-row">
    <label>Records: <span id="n-label">300</span></label>
    <input type="range" id="n-slider" min="0" max="300" value="300"
           oninput="document.getElementById('n-label').textContent=this.value">
    <button class="btn" onclick="generateData()">🔄 Generate</button>
    <button class="btn danger" onclick="resetData()">🗑️ Reset to 300</button>
  </div>

  <div class="sh">📈 Summary Statistics</div>
...
<!-- ═══════════ DIAGRAMS ═══════════ -->
<div id="panel-diagrams" class="panel">
  <div class="sh">① 5G Architecture Layers</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
graph TB
  UE[📱 User Equipment UE]-->RAN[🗼 5G RAN\nnext-gen NodeB gNB]
  ...
  <div class="sh">② Network Slicing Flow (3GPP TS 23.501)</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
flowchart LR
  REQ[Service\nRequest]-->NSSF[NSSF\nSlice Selection]
  ...
  <div class="sh">③ KNet Decision Flowchart</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
flowchart TD
  START([🚀 New 5G Deployment])-->UC{Use Case?}
  ...
  eMBB-->BAND{Band?}
  BAND-->|Dense urban|mmWave[mmWave 24-100 GHz\nSmall Cells]
  BAND-->|Suburban/Rural|Sub6[Sub-6 GHz\nMacro Cells]
  URLLC-->MEC2[Deploy MEC\n≤10ms to UE]
  mMTC-->LPWA[Low Power\nNarrow Band IoT]
  mmWave & Sub6 & MEC2 & LPWA-->KPI[Measure KPIs\nThroughput / Latency\nQoS / Energy]
  KPI-->PASS{KPI ≥ Target?}
  ...
  <div class="sh">④ 5G Core NF Interactions (SBA)</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
graph LR
  AMF[AMF\nAccess & Mobility]<-->SMF[SMF\nSession Mgmt]
  ...
  <div class="sh">⑤ QoS Score Formula Decomposition</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
flowchart LR
  A[Throughput\nMbps]-->|÷2000 × 40|W1[40% Weight]
  B[Latency\nms]-->|max0 10-L ÷10 × 30|W2[30% Weight]
  C[Availability\n%]-->|Avail-99 × 20|W3[20% Weight]
  D[Packet Loss\n%]-->|1-PL × 10|W4[10% Weight]
  W1 & W2 & W3 & W4-->SUM[Σ QoS Score\nclip 0–100]
    </div>
  </div>

  <div class="sh">⑥ 5G Deployment Lifecycle</div>
  <div class="mermaid-wrap">
    <div class="mermaid">
timeline
  title 5G Rollout Lifecycle (KNet Framework)
  Phase 1 : Spectrum Acquisition
           : Regulatory Approval
           : Site Survey
  Phase 2 : RAN Deployment
           : Core Network Setup
           : O-RAN Integration
...
<!-- ═══════════ FIELD DICTIONARY ═══════════ -->
<div id="panel-fields" class="panel">
  <div class="sh">📖 Field Definitions & Formulas</div>
  <div id="field-list"></div>
</div>

<!-- ═══════════ EXPORT ═══════════ -->
<div id="panel-export" class="panel">
  <div class="sh">📤 Export Filtered Dataset</div>
  <p style="color:#90CAF9;margin-bottom:20px;font-size:.9rem">
    Click a format below to download the current dataset.
  </p>
  <div class="export-grid">
    <div class="export-card" onclick="exportJSON()">
      <div class="export-icon">🗄️</div>
      <div class="export-lbl">JSON</div>
      <div class="export-sub">Structured + Metadata</div>
    </div>
    <div class="export-card" onclick="exportCSV()">
      <div class="export-icon">📋</div>
      <div class="export-lbl">CSV / Text</div>
      <div class="export-sub">Plain text tabular</div>
    </div>
  </div>
  <br>
  <div class="tax" style="margin-top:20px">
    <b>For PDF and Word (.docx) exports</b>, run the full Streamlit app
    (<code>knet_5g_framework.py</code>) locally with:<br><br>
    <code>pip install streamlit pandas numpy plotly reportlab python-docx</code><br>
    <code>streamlit run knet_5g_framework.py</code><br><br>
    Then use the Export tab in the Streamlit UI for rich PDF / DOCX output with full formatting,
    taxonomy header, field dictionary, and all 300 records.
  </div>
</div>

<!-- FOOTER -->
<div class="footer">
  © KNet 5G Intelligence Framework &nbsp;|&nbsp; Randy Singh &nbsp;|&nbsp;
  Kalsnet (KNet) Consulting Group &nbsp;|&nbsp;
  Built on 3GPP Rel-15/16/17 · ITU IMT-2020
</div>

<script>
// ════════════ DATA GENERATION ════════════
const REGIONS  = ["North America","Europe","Asia-Pacific","Middle East","Latin America","Africa"];
const BANDS    = ["Sub-6 GHz","mmWave (24-100 GHz)","FR1 (450 MHz–6 GHz)","FR2 (24.25–52.6 GHz)"];
const DEPLOY   = ["Standalone (SA)","Non-Standalone (NSA)","Hybrid"];
const USE_CASES= ["eMBB","URLLC","mMTC","FWA","Network Slicing","V2X","Private 5G"];
const OPERATORS= ["Verizon","AT&T","T-Mobile","Ericsson Net","Nokia Net","Huawei Net","Samsung Net","Rakuten"];
const VENDORS  = ["Ericsson","Nokia","Huawei","Samsung","ZTE","Mavenir","Rakuten Symphony"];
const SLICES   = ["eMBB Slice","URLLC Slice","mMTC Slice","Custom Slice"];
...
function generate(n=300,seed=42){
  const r=rng(seed), rows=[];
  for(let i=1;i<=n;i++){
    const band=pick(r,BANDS), uc=pick(r,USE_CASES);
    const hiBW=band.includes("mmWave")||band.includes("FR2");
    const tputMu=hiBW?1000:400;
    const tput=clamp(+rNorm(r,tputMu,tputMu*.2).toFixed(2),50,5000);
    const latMu=uc==="URLLC"?1:10;
    const lat=clamp(+Math.abs(rNorm(r,latMu,latMu*.3)).toFixed(2),0.5,200);
    const jitter=+Math.abs(rNorm(r,.5,.2)).toFixed(2);
    const pktLoss=+Math.abs(rNorm(r,.01,.005)).toFixed(4);
    const snr=+(5+r()*25).toFixed(1);
    const specEff=+(Math.log2(1+Math.pow(10,snr/10))).toFixed(2);
    const devDens=uc==="mMTC"?Math.floor(1000+r()*999000):Math.floor(100+r()*9900);
    const cov=hiBW?+(0.1+r()*1.9).toFixed(2):+(0.5+r()*9.5).toFixed(2);
    const pwr=10+r()*40;
    const enEff=+(tput/pwr).toFixed(2);
    const avail=+(99+r()*.999).toFixed(3);
    const qos=clamp(+(tput/2000*40+Math.max(0,(10-lat))/10*30+(avail-99)*20+(1-pktLoss)*10).toFixed(2),0,100);
    rows.push({
      Record_ID:`KNet-5G-${String(i).padStart(4,'0')}`,
      Region:pick(r,REGIONS),Operator:pick(r,OPERATORS),Vendor:pick(r,VENDORS),
      Frequency_Band:band,Deployment_Mode:pick(r,DEPLOY),Use_Case:uc,
      Network_Slice:pick(r,SLICES),Throughput_Mbps:tput,Latency_ms:lat,
      Jitter_ms:jitter,Packet_Loss_Pct:pktLoss,SNR_dB:snr,
      Spectral_Efficiency_bpHz:specEff,Device_Density_per_km2:devDens,
      Coverage_Radius_km:cov,Energy_Efficiency_MbpsW:enEff,
      Network_Availability_Pct:avail,QoS_Score:qos,
      Active_Sites:Math.floor(10+r()*4990),
      Year:[2023,2024,2025,2026][Math.floor(r()*4)]
    });
  }
  return rows;
}
...
function avg(arr,k){const v=arr.map(r=>+r[k]).filter(x=>!isNaN(x));return v.length?(v.reduce((a,b)=>a+b,0)/v.length):0}

function renderKPI(){
  const kpi=[
    [DATA.length,"Total Records"],
    [avg(DATA,"Throughput_Mbps").toFixed(0)+" Mbps","Avg Throughput"],
    [avg(DATA,"Latency_ms").toFixed(2)+" ms","Avg Latency"],
    [avg(DATA,"QoS_Score").toFixed(1),"Avg QoS Score"],
    [avg(DATA,"Network_Availability_Pct").toFixed(3)+"%","Avg Availability"],
    [avg(DATA,"Energy_Efficiency_MbpsW").toFixed(1),"Avg Energy Eff."],
  ];
  document.getElementById("kpi-row").innerHTML=kpi.map(([v,l])=>
    `<div class="kpi"><div class="kpi-val">${v}</div><div class="kpi-lbl">${l}</div></div>`
  ).join("");
}

function renderStats(){
  const numCols=["Throughput_Mbps","Latency_ms","QoS_Score","SNR_dB",
    "Spectral_Efficiency_bpHz","Energy_Efficiency_MbpsW","Network_Availability_Pct"];
  document.getElementById("stat-grid").innerHTML=numCols.map(k=>{
    const vals=DATA.map(r=>+r[k]).filter(x=>!isNaN(x)).sort((a,b)=>a-b);
    const mn=vals[0],mx=vals[vals.length-1],av=(vals.reduce((a,b)=>a+b,0)/vals.length);
    return `<div class="stat-card"><div class="stat-key">${k}</div>
      <div class="stat-val">${av.toFixed(2)}</div>
      <div style="color:#607D8B;font-size:.72rem">min ${mn.toFixed(2)} / max ${mx.toFixed(2)}</div></div>`;
  }).join("");
}
...
function renderTable(){
  if(!DATA.length){
    ...
    document.getElementById("tbody").innerHTML='<tr><td colspan="21" style="padding:20px;text-align:center;color:#607D8B">No records</td></tr>';
    return;
  }
  const cols=Object.keys(DATA[0]);
  document.getElementById("thead-row").innerHTML=cols.map(c=>`<th>${c}</th>`).join("");
  document.getElementById("tbody").innerHTML=DATA.slice(0,300).map(row=>
...
function generateData(){
  ...
  DATA=generate(n,42);
  renderKPI();renderStats();renderTable();
}
function resetData(){
  document.getElementById("n-slider").value=300;
  document.getElementById("n-label").textContent=300;
  DATA=generate(300,42);
  renderKPI();renderStats();renderTable();
}

// ════════════ FIELD DOCS ════════════
const FIELDS=[
  ["Record_ID","Unique identifier per site record.","KNet-5G-{NNNN} sequential"],
  ["Region","Geographic deployment region.","Categorical — 6 global regions"],
  ["Operator","Mobile Network Operator name.","Categorical — 8 major operators"],
  ["Vendor","Radio Access Network equipment vendor.","Categorical — 7 vendors"],
  ["Frequency_Band","Spectrum band for 5G transmission.","Categorical — 4 3GPP bands"],
  ["Deployment_Mode","SA = standalone 5GC; NSA = LTE anchor; Hybrid = mixed.","Per 3GPP Rel-15/16"],
  ["Use_Case","Primary 5G application type per ITU IMT-2020.","7 categories"],
  ["Network_Slice","Logical partition of 5G network (3GPP TS 23.501).","4 slice types"],
  ["Throughput_Mbps","Peak downlink data rate in Mbps.","Normal(μ=1000 if mmWave/FR2 else 400, σ=20%) ≥50"],
  ["Latency_ms","End-to-end round-trip delay in ms.","Normal(μ=1 if URLLC else 10, σ=30%) ≥0.5"],
  ["Jitter_ms","Packet Delay Variation (PDV).","| Normal(0.5, 0.2) |"],
  ["Packet_Loss_Pct","% of packets not delivered.","| Normal(0.01, 0.005) |"],
  ["SNR_dB","Signal-to-Noise Ratio in decibels.","Uniform(5, 30) dB"],
  ["Spectral_Efficiency_bpHz","Bits per second per Hz of bandwidth.","log₂(1 + 10^(SNR_dB/10))  [Shannon Capacity Theorem]"],
  ["Device_Density_per_km2","Connected devices per km².","Uniform(1K–1M) if mMTC, else Uniform(100–10K)"],
  ["Coverage_Radius_km","Cell radius for the deployment.","Uniform(0.1–2 km) if mmWave/FR2, else Uniform(0.5–10 km)"],
  ["Energy_Efficiency_MbpsW","Throughput per Watt — green 5G KPI.","Throughput_Mbps ÷ Uniform(10, 50) W"],
  ["Network_Availability_Pct","% uptime; 5-nines target = 99.999%.","Uniform(99.0, 99.999)"],
  ["QoS_Score","Composite Quality-of-Service index 0–100.","(T/2000×40) + (max(0,10−L)/10×30) + (A−99)×20 + (1−PL)×10, clipped [0,100]"],
  ["Active_Sites","Number of active base station sites.","Uniform Integer(10, 5000)"],
  ["Year","Deployment or measurement year.","Choice from [2023,2024,2025,2026]"],
];
document.getElementById("field-list").innerHTML=FIELDS.map(([n,d,f])=>
  `<div class="field"><span class="field-name">🔹 ${n}</span><br>${d}<br>
...
function exportJSON(){
  const payload={
    metadata:{framework:"KNet 5G Intelligence Framework",
      author:"Randy Singh",organization:"Kalsnet (KNet) Consulting Group",
      generated:new Date().toISOString(),records:DATA.length},
    taxonomy:{eMBB:"Enhanced Mobile Broadband — peak ≥20 Gbps",
      URLLC:"Ultra-Reliable Low-Latency — ≤1ms latency",
      mMTC:"Massive Machine-Type — 1M devices/km²"},
    data:DATA
  };
  ...
  a.download=`KNet5G_${Date.now()}.json`;a.click();
}
function exportCSV(){
  if(!DATA.length)return;
  const cols=Object.keys(DATA[0]);
  const rows=[
    "# KNet 5G Intelligence Framework | Randy Singh | Kalsnet (KNet) Consulting Group",
    cols.join(","),
    ...DATA.map(r=>cols.map(c=>`"${r[c]}"`).join(","))
  ];
  const blob=new Blob([rows.join("\n")],{type:"text/csv"});
  const a=document.createElement("a");a.href=URL.createObjectURL(blob);
  a.download=`KNet5G_${Date.now()}.csv`;a.click();
}

// ════════════ TAB SWITCHING ════════════
function showTab(id){
  document.querySelectorAll(".tab").forEach((t,i)=>{
    const ids=["overview","data","diagrams","fields","export"];
    t.classList.toggle("active",ids[i]===id);
  });
  document.querySelectorAll(".panel").forEach(p=>p.classList.remove("active"));
  document.getElementById("panel-"+id).classList.add("active");
  if(id==="diagrams") mermaid.run();
}
...
mermaid.initialize({
  ...
  theme:"dark",
  themeVariables:{
    primaryColor:"#1E90FF",primaryTextColor:"#E3F2FD",
    primaryBorderColor:"#1565C0",lineColor:"#64B5F6",
    background:"#0D2148",nodeBorder:"#1E90FF",
    clusterBkg:"#0A1628",edgeLabelBackground:"#0D2148"
  }
});
renderKPI(); renderStats(); renderTable();
</script>
