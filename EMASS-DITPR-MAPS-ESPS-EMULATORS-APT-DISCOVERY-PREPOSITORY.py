...
#EMASS-DITPR-MAPS-ESPS WITH REPOSITORY
...
# THIS MENU DRIVEN DATA EMULATOR (EMASS, DITPR, MAPS & ESPS) IS ASSEMBLED BY RANDY SINGH FROM DISA/EIIC/EM2 TO
# SUPPORT DISA OD1 AND OTHER API RELATED DISA MISSIONS.
...
def load_api_repository():
    global api_repository
    if os.path.exists('API-REPOSITORY.json'):
        with open('API-REPOSITORY.json', 'r') as f:
            api_repository = json.load(f)

def save_api_repository():
...
def main():
    ...
    # Display the API repository upon exiting
    print("\nFinal API Repository Content:")
    display_api_repository()
