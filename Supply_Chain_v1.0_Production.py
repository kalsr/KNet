...
Generate sample data, upload your SBOM, visualize risk levels, and download results.
""")
...
# ---------------------------
# 2. File Upload and Sample Generation
# ---------------------------
...
if st.button("Run AI Risk Detection"):
    ...
    df_proc['predicted_risk'] = pd.Categorical.from_codes(clf.predict(X), categories=['LOW','MEDIUM','HIGH'])
...
def highlight_risk(row):
    ...
    return [color_map.get(row['predicted_risk'], '') if col in ['package','version','supplier','license','predicted_risk'] else '' for col in row.index]
