...
if "data_volume" not in st.session_state:
    st.session_state.data_volume = 50
...
data_volume = st.sidebar.slider("Synthetic Data Volume", 0, 200, st.session_state.data_volume)
...
if st.sidebar.button("Reset & Refresh Data"):
    ...
    st.session_state.uploaded_file = None
    st.session_state.data_volume = 50
    st.info("Data has been reset. Please select mode and volume again.")
    st.experimental_rerun = None  # removed rerun to avoid AttributeError
    st.stop()  # Stop further execution safely
...
**1. Category**  
Operational severity: Low, Medium, High, Critical. Simulates SOC triage levels.

**2. Agentic Risk Score**  
AI probability score (1–100) of potential threat exposure.

**3. Confidence Score**  
Model confidence in Agentic Risk Score (50–100).

**4. MITRE Technique**  
Maps events to MITRE ATT&CK adversarial tactics for compliance & RMF readiness.

These fields demonstrate explainable enterprise AI analytics.
""")
