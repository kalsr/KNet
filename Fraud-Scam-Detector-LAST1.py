...
# ============================================================
# ENTERPRISE FRAUD & SCAM DETECTION PLATFORM (FINAL)
...
def generate_sample_image():
    ...
    d.text((50,250), generate_sample_image_text(), fill="black")
    return img

# ---------------- SESSION STATE ----------------
if "results" not in st.session_state:
    st.session_state.results = pd.DataFrame(
        columns=["Timestamp","Source","Content","RuleScore","ML","Confidence","Risk","Result","LLM_Explanation"]
    )
...
if "sample_messages" not in st.session_state:
    st.session_state.sample_messages = []
...
def llm_explain(text):
    if any(k in text.lower() for k in KEYWORDS):
        return "LLM Analysis: This message shows multiple scam indicators and is potentially fraudulent."
    else:
    ...
    if st.button("Analyze Message"):
        if text.strip():
            ...
            st.session_state.results.loc[len(st.session_state.results)] = (
                datetime.now(), "Text", text, score, ml, conf, risk, res, llm
            )
...
with tab2:
    ...
    with col1:
        ...
        if img_file:
            ...
            llm = llm_explain(text)
            st.session_state.results.loc[len(st.session_state.results)] = (
                datetime.now(), "Uploaded Screenshot", text, score, ml, conf, risk, res, llm
            )
    ...
    with col2:
        ...
        if st.button("Generate & Analyze Sample Screenshots"):
            for _ in range(n):
                ...
                llm = llm_explain(text)
                st.session_state.results.loc[len(st.session_state.results)] = (
                    datetime.now(), "Sample Screenshot", text, score, ml, conf, risk, res, llm
                )
                st.success(f"Sample Screenshot Analysis: {res} | Risk: {risk} | ML: {ml} | Confidence: {conf}%")
                st.info(llm)
...
with tab3:
    ...
    if st.button("Generate & Analyze Sample Messages"):
        for _ in range(n):
            ...
            st.session_state.results.loc[len(st.session_state.results)] = [
                datetime.now(), "Sample Text", msg, score, ml, conf, risk, res, llm
            ]
...
with tab4:
    if st.session_state.results.empty:
        ...
        df['Hour'] = df['Timestamp'].dt.floor('T')
        
        st.markdown("### Fraud vs Legitimate Overview")
        fig, ax = plt.subplots(figsize=(12,5))
        df["Result"].value_counts().plot(kind="bar", ax=ax, title="Fraud vs Legitimate")
        st.pyplot(fig)
        
        st.markdown("### Risk Bands")
        fig2, ax2 = plt.subplots(figsize=(12,5))
        df["Risk"].value_counts().plot(kind="pie", autopct="%1.1f%%", ax=ax2, title="Risk Distribution")
        st.pyplot(fig2)
        
        st.markdown("### ML Prediction Summary")
        fig3, ax3 = plt.subplots(figsize=(12,5))
        df["ML"].value_counts().plot(kind="bar", ax=ax3, title="ML Suspicious vs Normal")
        st.pyplot(fig3)
        
        st.markdown("### Average Confidence Score")
        st.metric("Avg Confidence", f"{df['Confidence'].mean():.2f}%")
        
        st.markdown("### Trend Over Time (Messages + Screenshots)")
        trend = df.groupby(['Hour','Source']).size().unstack(fill_value=0)
        st.line_chart(trend)
...
with tab5:
    if not st.session_state.results.empty:
        ...
        for _, r in st.session_state.results.iterrows():
            pdf.multi_cell(
                0,12,
                f"{r['Timestamp']} | {r['Source']} | {r['Result']} | Risk: {r['Risk']} | ML: {r['ML']} | Confidence: {r['Confidence']}%\nContent: {r['Content']}\nLLM: {r['LLM_Explanation']}\n"
            )
