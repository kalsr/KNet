# THIS MCP CYBER USE CASE IS DESIGNED BY RANDY SINGH FROM KNet CONSULTING GROUP.
...
# Manual Refresh
# -----------------------------
if 'manual_refresh_flag' not in st.session_state:
    st.session_state.manual_refresh_flag = False

def clear_data():
    for key in ['data', 'filtered_data', 'countdown']:
        if key in st.session_state:
            del st.session_state[key]
    st.session_state.manual_refresh_flag = True
...
if 'data' not in st.session_state:
    if uploaded_file:
        ...
        st.info("Upload a file or generate sample data to proceed.")
        df = None
else:
...
if df is not None:
    ...
    # Dashboard Layout
    # -----------------------------
    st.title("KNet Consulting - MCP Cyber Threat Dashboard")
    st.markdown("**Professional SOC-style monitoring dashboard with colored alert cards**")
    ...
    # -----------------------------
    # Alerts Tab (Severity-based)
    # -----------------------------
    with tabs[2]:
        st.subheader("Flagged Hosts / Alerts")
        flagged_hosts = filtered_data.copy()
        if not flagged_hosts.empty:
            for idx, row in flagged_hosts.iterrows():
