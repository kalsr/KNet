...
# EXPORT FUNCTIONS
# =========================================================================
# FPDF's built-in core fonts (Arial/Helvetica, etc.) only support the
# Latin-1 character set. Any emoji or special symbol in the data (e.g. the
# 🟢/🟡/🟠/🔴 condition markers, ₹, smart quotes, em-dashes) would previously
# crash export_to_pdf with a Unicode encode error, which is why PDF export
# silently failed. _pdf_safe() converts known symbols to readable plain-text
# equivalents and strips anything else that can't be rendered, so PDF
# generation always succeeds.
_PDF_CHAR_MAP = {
    "🟢": "[GOOD] ", "🟡": "[MONITOR] ", "🟠": "[WARN] ", "🔴": "[CRITICAL] ",
    "✅": "[OK] ", "⚠️": "[WARNING] ", "⚠": "[WARNING] ", "❌": "[X] ", "🔴": "[CRITICAL] ",
    "🛡️": "", "🔄": "", "📊": "", "📋": "", "📄": "", "📝": "", "🔧": "", "🔮": "", "⚙️": "", "📈": "",
    "₹": "Rs. ", "–": "-", "—": "-",
    "’": "'", "‘": "'", "“": '"', "”": '"', "…": "...",
}
...
def export_to_pdf(df: pd.DataFrame, title: str, formulas: Dict = None) -> bytes:
    ...
    try:
        ...
        pdf.cell(0, 5, "ShopFloor Sentinel | Kalsnet (KNet) Consulting", ln=True)
...
with tab1:
    ...
    if reset_demo:
        ...
        st.success("✅ Synthetic defect data reset")
...
with tab2:
    ...
    with col4:
        st.write("")
        st.write("")
        reset_tools = st.button("🗑️ Reset", key="reset_tools")

    if reset_tools:
        st.session_state.tool_df = None
        st.success("✅ Synthetic tool wear data reset")
