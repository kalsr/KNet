...
# =============================================================================
# This dictionary is the single source of truth for the framework's structure.
# It powers the UI tabs, the diagrams, and every export format.
...
TAXONOMY = {
    "L1": {
        ...
        "categories": [
            {
                "name": "Low-Band (Sub-1 GHz)",
                "detail": "Frequencies below 1 GHz (e.g., 600–850 MHz). Offers the widest coverage and best building penetration but lower peak throughput. Used for broad rural and indoor coverage."
            },
            {
                "name": "Mid-Band (1–6 GHz / FR1)",
                "detail": "Often called 'C-Band' (e.g., 3.3–4.2 GHz). Balances coverage and capacity — the workhorse band for most nationwide 5G deployments."
            },
            {
                "name": "High-Band / mmWave (FR2, 24–100 GHz)",
                "detail": "Millimeter wave spectrum delivering multi-Gbps peak speeds over very short range; highly susceptible to obstruction and rain fade. Used for dense urban hotspots and fixed wireless access."
            },
            {
                "name": "Shared & Unlicensed Spectrum",
                "detail": "CBRS (3.5 GHz in the US) and unlicensed bands enabling private 5G networks and neutral-host deployments without exclusive licensing."
            },
        ]
    },
    "L2": {
        ...
        "categories": [
            {
                "name": "gNodeB (gNB)",
                "detail": "The 5G base station — the radio access node that terminates the air interface (Uu) toward the User Equipment (UE) and connects to the 5G Core."
            },
            {
                "name": "Massive MIMO & Beamforming",
                "detail": "Antenna arrays with dozens/hundreds of elements that steer focused signal beams toward users, multiplying capacity and range without new spectrum."
            },
            {
                "name": "Open RAN (O-RAN)",
                "detail": "A disaggregated RAN architecture splitting baseband (CU/DU) and radio (RU) functions across open, interoperable interfaces, reducing vendor lock-in."
            },
            {
                "name": "Non-Standalone (NSA) vs Standalone (SA)",
                "detail": "NSA anchors 5G radio to an existing 4G LTE core for control signaling; SA uses a fully native 5G Core end-to-end, unlocking low latency and slicing."
            },
        ]
    },
    "L3": {
        ...
        "categories": [
            {
                "name": "AMF — Access & Mobility Management Function",
                "detail": "Handles registration, connection, reachability, and mobility management for the UE; the first point of contact in the 5G Core."
            },
            {
                "name": "SMF — Session Management Function",
                "detail": "Establishes, modifies, and releases PDU (Packet Data Unit) sessions, allocating IP addresses and selecting the UPF."
            },
            {
                "name": "UPF — User Plane Function",
                "detail": "Routes and forwards actual user data traffic; can be deployed at the network edge to minimize latency."
            },
            {
                "name": "Service-Based Architecture (SBA)",
                "detail": "5GC functions expose and consume APIs over HTTP/2, enabling modular, independently scalable microservices instead of monolithic network elements."
            },
        ]
    },
    "L4": {
        ...
        "categories": [
            {
                "name": "eMBB Slice (Enhanced Mobile Broadband)",
                "detail": "Optimized for high data throughput — video streaming, AR/VR, broadband-like consumer experiences."
            },
            {
                "name": "URLLC Slice (Ultra-Reliable Low-Latency Communication)",
                "detail": "Optimized for sub-5ms latency and 99.999%+ reliability — autonomous vehicles, remote surgery, industrial automation."
            },
            {
                "name": "mMTC Slice (Massive Machine-Type Communication)",
                "detail": "Optimized for connection density — supporting up to ~1 million devices per km² for IoT sensors and smart-city deployments."
            },
            {
                "name": "Network Functions Virtualization (NFV)",
                "detail": "Decouples network functions from dedicated hardware, running them as software instances on commodity infrastructure (NFVI)."
            },
        ]
    },
    "L5": {
        ...
        "categories": [
            {
                ...
                "detail": "Places compute and storage resources physically near the radio edge, cutting round-trip latency for time-critical applications."
            },
            {
                "name": "MANO (Management & Orchestration)",
                "detail": "ETSI NFV-MANO framework coordinating the lifecycle of virtual network functions (VNFs) — onboarding, scaling, healing."
            },
            {
                "name": "Closed-Loop Automation / AI-RAN",
                "detail": "Self-optimizing networks (SON) using AI/ML to detect anomalies and auto-remediate without human intervention."
            },
            {
                "name": "Network Slicing Orchestration",
                "detail": "Coordinates end-to-end slice lifecycle across RAN, transport, and core to meet a tenant's committed SLA."
            },
        ]
    },
    "L6": {
        ...
        "categories": [
            {
                "name": "SUPI / SUCI Concealment",
                "detail": "The Subscription Permanent Identifier is never sent in the clear; it is encrypted as a SUCI (Subscription Concealed Identifier) to prevent IMSI-catcher attacks."
            },
            {
                "name": "Zero Trust Network Segmentation",
                "detail": "Each network function and slice is treated as untrusted by default, requiring mutual authentication (typically via TLS and OAuth2) for every API call."
            },
            {
                "name": "Security Edge Protection Proxy (SEPP)",
                "detail": "Filters and protects signaling traffic exchanged between operators' 5G Cores at network boundaries (the N32 interface)."
            },
            {
                "name": "Lawful Intercept & Privacy Compliance",
                "detail": "Regulatory-mandated capabilities for authorized monitoring, balanced against subscriber privacy protections (e.g., GDPR-aligned data handling)."
            },
        ]
    },
    "L7": {
        ...
        "categories": [
            {
                ...
                "detail": "5G-based home/business broadband replacing fiber or cable in markets where wired infrastructure is costly to deploy."
            },
            {
                "name": "Private 5G / Enterprise Networks",
                "detail": "Dedicated 5G networks (often on CBRS or licensed spectrum) deployed for a single enterprise — factories, ports, campuses."
            },
            {
                "name": "Connected Vehicles & V2X",
                "detail": "Vehicle-to-Everything communication enabling cars to talk to infrastructure, other vehicles, and pedestrians using URLLC slices."
            },
            {
                "name": "Smart Manufacturing / Industry 4.0",
                "detail": "Combines mMTC sensor density with URLLC control loops to enable wireless robotics and predictive maintenance on the factory floor."
            },
        ]
    },
}

# Field-level glossary describing each column in the synthetic 5G record dataset
FIELD_GLOSSARY = [
...
# Key formulas / quantitative concepts used across the framework
FORMULAS = [
    {
        "name": "Shannon Capacity (Theoretical Channel Capacity)",
    ...
    },
    {
        ...
        "formula": "η = Throughput (bps) / Bandwidth (Hz)",
    ...
    {
        "name": "Latency Budget (URLLC)",
        "formula": "T_total = T_air + T_proc + T_transport + T_core",
    ...
    {
        "name": "Network Slice Capacity Allocation",
        "formula": "Σ(slice_i resource share) ≤ 100% of RAN/Core resource pool",
    ...
    {
        "name": "Connection Density (mMTC)",
        "formula": "Density = Devices / km²",
    ...
    {
        "name": "Reliability Target (URLLC)",
        "formula": "Reliability = (Successful Packets / Total Packets) × 100%",
        "explain": "IMT-2020 URLLC target is 99.999% (the 'five nines'), meaning no more than 1 failure in 100,000 transmissions."
    },
]

# 5QI sample mapping (simplified, illustrative subset for educational purposes)
QOS_5QI_TABLE = [
...
def generate_synthetic_5g_data(n_records: int, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(n_records):
        ...
        # Latency / throughput / reliability vary realistically by slice type
        if slice_type == "URLLC":
...
# Mermaid diagram definitions (one per conceptual flow)
MERMAID_TAXONOMY_TREE = """
flowchart TB
...
with st.sidebar:
    ...
    if reset_clicked:
        reset_all_data()
        st.toast("Sample data and controls have been reset.", icon="")
        st.rerun()
...
""")
    ...
    with col1:
        st.markdown('<div class="layer-badge"> 7 Structural Layers</div>', unsafe_allow_html=True)
...
5. **Export & Reports tab** — download the complete framework (including any generated sample data) as **PDF, Word (DOCX), Text, or JSON**.
""")
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab_diagrams:
    ...
    st.markdown(
        "Shows the simplified message flow when a device (UE) attaches to the network and establishes a "
    ...
    st.markdown(
        "Illustrates how one shared physical infrastructure is logically divided by the orchestration layer "
    ...
    st.markdown(
        "Shows how subscriber identity is protected (SUCI/SUPI), how internal functions authenticate under "
...
def build_export_payload():
    """Assembles the complete framework + sample data into one structured dict,
    used as the canonical source for JSON, TXT, DOCX, and PDF exports."""
    df = st.session_state.synthetic_df
...
def export_txt(payload: dict) -> bytes:
    ...
    hr("-")
    lines.append("SECTION 3: SAMPLE 5G RECORD FIELD GLOSSARY".center(width))
    hr("-")
    ...
    hr("-")
    lines.append("SECTION 4: KEY FORMULAS".center(width))
    hr("-")
    ...
    hr("-")
    lines.append("SECTION 5: GLOSSARY OF TERMS".center(width))
    hr("-")
    ...
    hr("-")
    lines.append(f"SECTION 6: SAMPLE 5G RECORDS (n={len(payload['sample_5g_records'])})".center(width))
    hr("-")
...
def export_docx(payload: dict) -> bytes:
    ...
    # ---- Section 3: Field glossary ----
    h3 = doc.add_heading("3. Sample 5G Record — Field-by-Field Glossary", level=1)
    for run in h3.runs:
        run.font.color.rgb = navy
    ...
    # ---- Section 4: Formulas ----
    h4 = doc.add_heading("4. Key Formulas Behind 5G Performance", level=1)
    for run in h4.runs:
        run.font.color.rgb = navy
    for f in payload["formulas"]:
    ...
    # ---- Section 5: Glossary ----
    h5 = doc.add_heading("5. Glossary of Terms", level=1)
    for run in h5.runs:
        run.font.color.rgb = navy
    ...
    h6 = doc.add_heading(f"6. Sample 5G Records (n={len(payload['sample_5g_records'])})", level=1)
    for run in h6.runs:
        run.font.color.rgb = navy
    ...
    if records:
        ...
        max_rows = min(len(records), 150)  # keep docx manageable
...
def export_pdf(payload: dict) -> bytes:
    ...
    # ---- Section 3: Field glossary ----
    elements.append(Paragraph("3. Sample 5G Record — Field-by-Field Glossary", style_h1))
    ...
    # ---- Section 4: Formulas ----
    elements.append(Paragraph("4. Key Formulas Behind 5G Performance", style_h1))
    for f in payload["formulas"]:
    ...
    # ---- Section 5: Glossary ----
    elements.append(Paragraph("5. Glossary of Terms", style_h1))
    ...
    # ---- Section 6: Sample records ----
    records = payload["sample_5g_records"]
    elements.append(Paragraph(f"6. Sample 5G Records (n={len(records)})", style_h1))
    if records:
        max_rows = min(len(records), 80)  # keep PDF a reasonable size
        cols = list(records[0].keys())
...
field-by-field glossary, formulas, terminology dictionary, and any currently generated sample 5G records —
in your preferred format. Every export carries the **{BRAND_NAME}** title block and attribution to
**{AUTHOR_NAME}**.
""")
