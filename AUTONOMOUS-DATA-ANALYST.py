...
def data_cleaner(df, report):
    ...
    # Simple cleaning: trim column names, infer dtypes, fill small missing values
    ...
    # Fill numeric small missing with median
...
def modeling_agent(df, report, target_column=None):
    ...
    # If a numeric target exists, try a simple linear regression with top numeric feature

    numeric = df.select_dtypes(include=[np.number]).copy()

    if target_column is None:

        # try to find a binary or numeric 'will_churn' or 'target' or 'revenue_30d'
...
# Simple CSS for colored buttons
...
.blue {background: linear-gradient(90deg,#3498DB,#2E86C1);}

.orange {background: linear-gradient(90deg,#F39C12,#D68910);}
...
with col1:
    ...
    st.markdown('<div class="big-button blue small">Status: Idle</div>', unsafe_allow_html=True)

with col2:

    if st.button("Generate 100 Sample Records"):

        df_generated = generate_sample(100)

        st.session_state["df"] = df_generated

        st.success("Generated 100 sample records (stored in-memory).")
...
with colC:
    ...
    export_btn = st.button("Download Processed Data / Report", key="export")
