...
def load_data(uploaded_file):
    try:
        ...
        # 3. FALLBACK DATA (never fail)
        else:
            st.warning("Dataset not found. Using fallback sample data.")

            data = {
                "country_long": ["Iran"] * 12,
                "name": [f"Plant_{i}" for i in range(12)],
                "primary_fuel": ["Gas","Oil","Hydro","Solar","Wind","Gas","Oil","Hydro","Solar","Wind","Gas","Oil"],
                "capacity_mw": [100,200,150,50,75,300,250,120,80,60,180,220],
                "latitude": [32,31,30,29,28,27,26,25,24,23,33,34],
                "longitude": [52,51,50,49,48,47,46,45,44,43,53,54]
            }
...
if len(df) > 0:
    ...
    st.write(f"Coverage: {coverage:.2f}% of dataset after filtering")
...
# ---------------- FOOTER ----------------
st.markdown("---")
st.caption("KNet Intelligence Platform | Energy & Infrastructure Analytics")