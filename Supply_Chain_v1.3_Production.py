...
# Page config
# ---------------------------
st.set_page_config(page_title="KNet AI SBOM Monitor", layout="wide")
st.title("KNet AI SBOM / Supply-Chain Monitor (Production Ready)")
st.markdown("""
...
display_df['risk_badge'] = display_df.get('predicted_risk','LOW').apply(render_badge)
...
if 'predicted_risk' in display_df.columns:
    pie_data = display_df['predicted_risk'].fillna('LOW').value_counts().reset_index()
    ...
    display_df['timestamp_dt'] = display_df['timestamp_dt'].fillna(pd.Timestamp.now())
