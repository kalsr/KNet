...
cursor.execute("CREATE TABLE IF NOT EXISTS activities (id INTEGER PRIMARY KEY, description TEXT, timestamp TEXT)")
conn.commit()
...
def log_call(call_type):
    ...
    conn.commit()
    messagebox.showinfo("Call Logged", f"{call_type} logged at {time}")
...
def show_logs():
    ...
    logs = "\n".join([f"Call: {c[1]} at {c[2]}" for c in calls] + [f"Activity: {a[1]} at {a[2]}" for a in activities])
...
# Buttons
tk.Button(button_frame, text="📞 Log Incoming Call", bg="#90ee90", font=("Helvetica", 14), width=20, command=lambda: log_call("Incoming")).grid(row=0, column=0, padx=10, pady=10)
tk.Button(button_frame, text="❌ Log Missed Call", bg="#ffcccb", font=("Helvetica", 14), width=20, command=lambda: log_call("Missed")).grid(row=0, column=1, padx=10, pady=10)
tk.Button(button_frame, text="📋 Show Logs", bg="#add8e6", font=("Helvetica", 14), width=20, command=show_logs).grid(row=0, column=2, padx=10, pady=10)
