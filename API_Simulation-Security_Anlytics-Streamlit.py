...
# ------------------------------------------------------------
# Synthetic Data Generator (VARIABLE ANOMALIES)
# ------------------------------------------------------------
def generate_synthetic_data(n, seed):
    random.seed(seed)

    endpoints = [
        "/api/login", "/api/data", "/api/user",
        "/api/admin", "/api/delete", "/api/update", "/api/unknown"
    ]
    statuses = ["200", "401", "403", "404", "500"]

    records = []

    anomaly_probability = random.choice([0.0, 0.2, 0.4, 0.6])
