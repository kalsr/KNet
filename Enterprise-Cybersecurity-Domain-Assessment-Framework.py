...
""", unsafe_allow_html=True)
with st.expander("About This Application — How It Works", expanded=True):
    st.markdown(f"""
...
if page =="Executive Dashboard":
    ...
    with r1c1:
        ...
        fig.update_layout(title="NIST CSF 2.0 Function Maturity Radar",
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        if not REPORTLAB_OK:
            ...
            if data:
                st.download_button("Download PDF", data=data, file_name=fname,
    ...
    with c2:
        ...
        if not DOCX_OK:
            ...
            if data:
                st.download_button("Download Word", data=data, file_name=fname,
                                   mime=mime, use_container_width=True, type="primary")
    with c3:
        ...
        st.download_button("Download Text", data=data, file_name=fname,
                           mime=mime, use_container_width=True, type="primary")
    with c4:
        st.markdown("#### JSON Data")
        st.caption("Machine-readable full assessment for GRC tool import or APIs.")
        data, fname, mime = export_json(df)
        st.download_button("Download JSON", data=data, file_name=fname,
                           mime=mime, use_container_width=True, type="primary")
    st.divider()
