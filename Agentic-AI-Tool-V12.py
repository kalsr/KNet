...
# ----------------------------------------------------------
# ✅ NEW: LLM + AGENTIC AI EXPLANATION
# ----------------------------------------------------------
st.markdown("---")
st.subheader(" How Autonomous Agentic AI Engine Works")

st.markdown("""
###  LLM (Large Language Model) Engine
This platform uses a Large Language Model (LLM) via the GROQ API to simulate intelligent reasoning.  
The LLM processes human language inputs and performs:
...
The system behaves like an **autonomous AI agent**, meaning:

1. Takes user intent (task input)  
2. Interprets the objective automatically  
3. Generates insights without predefined rules  
4. Adapts based on selected use case  

This removes the need for manual analysis.

---

###  Analytics Engine
Once AI generates insights, the platform automatically:

- Creates structured datasets  
- Simulates real-world operational data  
- Calculates risk levels dynamically  
- Produces consistent summaries  

This represents the **data intelligence layer**.
...
- Downloadable outputs (CSV, JSON, PDF)  

This enables **decision-making and reporting**.

---

###  End-to-End Flow
User Input → AI Reasoning → Data Generation → Risk Scoring → Visualization → Report Export  

This is a full **Autonomous AI + Analytics + Reporting pipeline**.
""")
...
# ✅ NEW: AUTO-FILL TASK BASED ON USE CASE
task = st.text_area(
    "Enter your task:",
    value=f"Analyze {final_use_case} and generate insights, risk analytics, and recommendations."
)
