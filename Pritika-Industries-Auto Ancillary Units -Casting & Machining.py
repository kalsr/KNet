...
# ----------------------------------------------------------------------------
    ...
    **Process**
    ...
    Each row reflects the typical before-versus-after delta reported across the six modules in
    this app once deployed on a live line: Vision QC drives the rejection-rate and
    complaint-reduction rows, Predictive Maintenance drives downtime, Tool Wear drives tool cost,
    Demand Forecast drives inventory/working capital, Energy Optimization drives the energy-bill
    row, and Traceability drives RCA time.
...
2. **Sensor retrofit** - vibration and current sensors on the top 3 bottleneck machines
   (approximately Rs. 50,000 per machine).
3. **Use existing data** - CMM logs, rejection logs, and tool-change sheets feed the first
   predictive models.
4. **Scale to scheduling, demand forecasting, and energy** once the shop floor trusts the QC and
   maintenance pilots.
        """
    )
    ...
    formulas_dict = {
        ...
        "Methodology": (
            "Before/after deltas aggregated from the Vision QC, Predictive Maintenance, Tool "
            "Wear, Demand Forecast, Energy, and Traceability modules."
        ),
    }
