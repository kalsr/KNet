...
# Fraud-Detector-V5
# app.py - Enhanced Fraud/Scam Detector — Full feature set
...
# ---------------- top controls (Option 1) ----------------
st.markdown("<div class='top-bar'>"
            "<a class='top-btn gen' href='?page=home'>Home</a>"
            "<a class='top-btn gen' href='?page=analysis'>Analysis</a>"
...
def generate_sample_data(n):
    """Generate sample tabular transactions with a small fraud column for metrics/demos."""
    ...
    # lightweight synthetic fraud label (for demo only)
...
if page == "sample":
    ...
    # Generate button triggers creation using the slider value
    ...
    # Allow download of the generated sample CSV
...
if page == "analysis":
    ...
    st.write("Run unsupervised anomaly detection (IsolationForest) and compute heuristic scores.")
    df = st.session_state.df.copy()
    # We expect columns: amount, device_changes, ip_distance, velocity_score. If missing, try to derive.
    required_cols = ["amount", "device_changes", "ip_distance", "velocity_score"]
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        st.warning(f"Dataset missing columns: {missing}. The analysis uses available numeric columns where possible.")
    # Fill missing numeric columns with defaults if needed
    for c in required_cols:
        if c not in df.columns:
            df[c] = 0

    # Heuristic scoring (same rules described earlier)
    ...
    # ML Score: use simple scaled transformation of velocity and amount for demo
    ...
    features_for_iso = df[["amount", "device_changes", "ip_distance", "velocity_score"]].astype(float)
    try:
        iso.fit(features_for_iso)
        df["iso_pred"] = iso.predict(features_for_iso)  # -1 anomaly, 1 normal
        # Flag anomalies
    ...
    st.markdown("**ML Score calculation:** scaled combination of `velocity_score` and `amount` — presented 0–100.  \n"
                f"**ML Score (calculated):** **{ml_score_mean:.2f}**")

    st.markdown("**Heuristic Score calculation:** rule-based weights (amount/device changes/ip distance/velocity).  \n"
                f"**Heuristic Score (calculated):** **{heuristic_mean:.2f}**")

    st.markdown("**Combined Score calculation:** `Combined = 0.6 * ML + 0.4 * Heuristic`.  \n"
                f"**Combined Score (calculated):** **{combined_mean:.2f}**")
    ...
    if not flagged_df.empty:
        ...
        st.info("No anomalies flagged by IsolationForest at the configured contamination level.")

    st.markdown("---")
    st.subheader("Visualizations")
    # bar chart
    fig = px.bar(x=["ML Score", "Heuristic Score", "Combined Score"], y=[ml_score_mean, heuristic_mean, combined_mean],
                 title="Average Scores", color=[ml_score_mean, heuristic_mean, combined_mean],
    ...
    fig2 = px.pie(values=pie_sizes, names=pie_labels, title="Score Composition (avg)", color=pie_labels,
                  color_discrete_map={"ML":"#1f77b4", "Heuristic":"#ff7f0e", "Remaining":"#2ca02c"})
    st.plotly_chart(fig2, use_container_width=True)

    # Heatmap (correlation)
    st.subheader("Feature correlation heatmap")
    corr = df[["amount", "device_changes", "ip_distance", "velocity_score", "combined_score"]].corr()
...
# ---------------- PAGE: ML METRICS (supervised if label exists) ----------------
if page == "ml":
    st.header("ML Metrics & Model Evaluation")
    df = st.session_state.df.copy()
    # Check for label
    if "is_fraud" not in df.columns:
        st.info("No 'is_fraud' label column present. For supervised metrics (confusion matrix, ROC), upload a labeled dataset with 'is_fraud'.")
    else:
        X = df[["amount", "device_changes", "ip_distance", "velocity_score"]].astype(float)
        y = df["is_fraud"].astype(int)
        # Train/test simple RandomForest for demo metrics
        from sklearn.model_selection import train_test_split
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42, stratify=y)
        clf = RandomForestClassifier(n_estimators=200, random_state=42)
        with st.spinner("Training RandomForest for metrics..."):
...
# ---------------- PAGE: HEATMAP ----------------
if page == "heatmap":
    st.header("Fraud Heatmap / Keyword heatmaps (if logs provided)")
    df = st.session_state.df.copy()
    # If dataset is tabular, show feature heatmap (correlation already available)
    if set(["amount","device_changes","ip_distance","velocity_score"]).issubset(df.columns):
        corr = df[["amount","device_changes","ip_distance","velocity_score","combined_score"]].corr()
        fig, ax = plt.subplots(figsize=(7,5))
        sns.heatmap(corr, annot=True, cmap="mako", ax=ax)
        st.pyplot(fig)
    else:
        st.info("No numeric transaction columns available to build a heatmap.")
...
if page == "export":
    ...
    st.write("Download analysis artifacts below:")

    if st.button("Generate PDF Report"):
        with st.spinner("Generating PDF..."):
            pdf_bytes = generate_pdf_report_bytes(df, ar, st.session_state.get("llm_output"))
            st.success("PDF generated")
            st.download_button("Download Report PDF", data=pdf_bytes, file_name=f"Fraud_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", mime="application/pdf")
            st.balloons()

    if st.button("Download full dataset CSV"):
        st.download_button("Download CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="dataset.csv", mime="text/csv")

    st.markdown("---")
    st.subheader("LLM Analysis (optional)")
    st.markdown("You can request a natural-language analysis from an LLM. This will be attempted only if an LLM library is installed and corresponding API key is configured in the environment (e.g. OPENAI_API_KEY).")
    log_text = st.text_area("Optional: paste logs or text for LLM analysis", height=180)
    if st.button("Run LLM Analysis (best-effort)"):
        if not log_text.strip():
            # fallback: use summary of top flagged events
            sample = df.head(50).to_string()
            log_text = f"Top events:\n{sample}"
        with st.spinner("Calling LLM (if configured)... this may take a while"):
            llm_out = run_llm_analysis_text(log_text)
            st.session_state.llm_output = llm_out
            st.text_area("LLM output", value=llm_out, height=300)

# ---------------- persistent Reset button bottom-right (non-destructive) --------------
