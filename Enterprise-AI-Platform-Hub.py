...
if st.session_state.reset_requested:
    ...
    st.session_state.uploaded_file = None
    st.session_state.reset_requested = False
    st.experimental_rerun()  # Safe here because it is after all initialization
...
if st.session_state.df is None:
    if st.session_state.uploaded_file:
        if st.session_state.uploaded_file.name.endswith(".csv"):
            ...
            st.session_state.df = pd.read_json(st.session_state.uploaded_file)
        st.success("User Dataset Loaded Successfully")
    else:
        st.session_state.df = generate_data(data_volume)
...
**1. Category**  
Represents the operational severity classification. Categories (`Low`, `Medium`, `High`, `Critical`) simulate SOC triage levels for risk visualization.

**2. Agentic Risk Score**  
AI-generated probability score (1–100) for threat exposure per record. Higher indicates elevated risk requiring mitigation.

**3. Confidence Score**  
Model confidence (50–100) in the Agentic Risk Score. Higher = stronger supporting evidence.

**4. MITRE Technique**  
Maps events to MITRE ATT&CK adversarial tactics, showing compliance & RMF readiness integration.

Collectively demonstrates enterprise AI analytics, explainability, compliance, and executive risk visibility.
""")
