...
def highlight_risk(row):
    ...
    return [color_map.get(str(row.get('predicted_risk','LOW')), '') 
            if col in ['package','version','supplier','license','predicted_risk'] else '' 
            for col in row.index]

st.dataframe(display_df.fillna('').style.apply(highlight_risk, axis=1), use_container_width=True)
