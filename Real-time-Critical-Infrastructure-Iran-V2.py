...
# ---------------- HEADER ----------------
st.markdown(
    "<h1 style='text-align:center;color:blue;font-weight:bold;'>Energy Infrastructure Intelligence Dashboard</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<h3 style='text-align:center;color:blue;font-weight:bold;'>Developed by Randy Singh - Kalsnet (KNet) Consulting Group</h3>",
    unsafe_allow_html=True
)
...
# ==========================================================
# 🔥 REALISTIC DATA GENERATOR (200 RECORDS WITH location_name)
# ==========================================================
def generate_realistic_data(n=200):
    ...
    df = pd.DataFrame({
        ...
        "location_name": [f"City_{i}" for i in range(1, n + 1)]
    })
    return df
...
# Allow user to select number of records
max_records = st.sidebar.slider("Number of Records to Display", 1, len(df), 50)
...
if not filtered_df.empty and "latitude" in filtered_df.columns and "longitude" in filtered_df.columns:
    fig = px.scatter_mapbox(
        ...
        size="capacity_mw",
        hover_name="name",
        hover_data=["location_name"] if "location_name" in filtered_df.columns else None,
        zoom=4
    )
...
desired_columns = ["name", "primary_fuel", "capacity_mw", "latitude", "longitude", "location_name"]
...
st.markdown("### Download Full Generated Dataset")
generated_csv = df.to_csv(index=False).encode("utf-8")
st.download_button("Download Current Dataset", generated_csv, "generated_power_plants.csv")
...
def generate_pdf(data):
    ...
    for _, row in data.iterrows():
        row_text = " | ".join([str(row[col]) for col in existing_columns])
