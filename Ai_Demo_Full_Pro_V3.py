# Ai_Demo_Full_Pro_Fixed_Colored.py
...
.download-csv {background-color: #28a745;}
.download-pdf {background-color: #ffc107;}
.reset-btn {background-color: #dc3545;}
.generate-btn {background-color: #17a2b8;}
</style>
...
def download_csv(df):
    ...
    st.download_button("⬇️ Download CSV", data=csv, file_name="results.csv", mime="text/csv", key="download_csv")
...
def download_pdf(df, title="AI Application Results"):
    ...
    st.download_button("📄 Download PDF", data=pdf_output, file_name="results.pdf", mime="application/pdf", key="download_pdf")
...
if menu == "🏁 Home":
    ...
    uploaded = st.file_uploader("Upload your dataset (CSV, Excel, JSON)", type=["csv","xlsx","json"], key="file_uploader")
    ...
    if st.button("🔁 Generate Synthetic Data", key="generate_btn"):
        if "Machine Learning" in app_name:
            st.session_state.df = generate_classification(n_records)
        elif "NLP" in app_name:
            st.session_state.df = generate_text(n_records)
            ...
            st.session_state.df = pd.DataFrame({"Step": np.arange(n_records), "Reward": np.cumsum(np.random.randn(n_records))})
        elif "Data & Preprocessing" in app_name:
            st.session_state.df = generate_regression(n_records)
    ...
    # --- Speech Playback ---
    if "Speech" in app_name and st.session_state.df is not None:
    ...
    if st.session_state.df is not None:
        ...
        download_pdf(st.session_state.df, title=f"{app_name} Results - KNet Consulting Group")
