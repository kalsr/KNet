...
st.markdown("Compare any two publicly traded companies using real-time Yahoo Finance data. "
            "Includes ratios, margins, price history, and report exports. **Not financial advice.**")
...
score_df = df.copy()
score_df['net_margin_norm'] = minmax_norm(score_df['net_margin'])
score_df['dividend_norm'] = minmax_norm(score_df['dividend_yield_pct'])
score_df['ey_norm'] = minmax_norm(score_df['earnings_yield'])
score_df['pe_norm'] = minmax_norm(score_df['pe'], invert=True)
score_df['rev_norm'] = minmax_norm(score_df['revenue_bil'])
...
st.markdown("---")
st.caption("This tool is designed by Randy Singh from KNet consulting Group & uses Yahoo Finance data. Educational use only. Not financial advice.")
